  ...
  return (
    <header style={{ backgroundColor: '#f5f5f5', borderBottom: 'none' }}>
      <div className="max-w-[900px] mx-auto px-5 py-5 flex items-center justify-between">
        <Link href="/" className="font-bold tracking-tight hover:opacity-60 transition-opacity flex-shrink-0" style={{ color: '#111827', fontSize: '17px' }}>
          tr.dincer
        </Link>

        <nav className="flex items-center gap-4">
          {/* Nav linkleri sadece tablet+ görünür */}
          <div className="hidden sm:flex items-center gap-5">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-medium transition-colors hover:opacity-60 whitespace-nowrap"
